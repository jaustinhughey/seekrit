...
Vagrant.configure(2) do |config|
  ...
  config.vm.provision "shell", inline: <<-SHELL
    apt-get update
    apt-get upgrade -y
    apt-get install -y build-essential libffi-dev libgdbm-dev libncurses5-dev \
    libreadline-dev libssl-dev libyaml-dev zlib1g-dev tmux vim git \
    libsqlite3-dev redis-server

    # Update the hostname so we know what project we're in
    hostname SEEKRIT
    ...
    su vagrant -c 'source ~/.profile && rbenv install 2.2.0 && rbenv global 2.2.0'
